...
"""
    ...
    async def run_pipeline(
        ...
        def start_stage(number: int, name: str) -> float:
            ...
            set_stage(progress_run_id, progress_case_id, number, "running", f"{name} 진행 중")
        ...
        def finish_stage(number: int, name: str, started: float) -> float:
            ...
            set_stage(
                progress_run_id, progress_case_id, number, "done",
                f"{name} 완료 · {elapsed:.2f}초",
            )
        ...
        if not texts:
            ...
            skip_stages(progress_run_id, progress_case_id, 3, "관련 VOC 데이터가 없어 실행하지 않음")
            return {
                "summary": "",
                "trace": "; ".join(trace),
                "ok": False,
...
class SummarizerServicer(voc_pb2_grpc.SummarizerServicer):
    ...
    async def RunPipeline(self, request, context):
        ...
        try:
            ...
            fail_active_stage(progress_run_id, progress_case_id, str(e))
            # ============ 에러 처리 ============
            # 예외 발생 시 gRPC 에러로 변환하여 클라이언트에 전달합니다
            await context.abort(
                grpc.StatusCode.INTERNAL,  # 내부 서버 오류 상태 코드
