...
from allstar.shared.paths import PACKAGE_ROOT, VOC_DATA_ROOT, VOC_REPORT_ROOT
...
ROOT = PACKAGE_ROOT / "voc"
REPORTS_DIR = VOC_REPORT_ROOT / "testcase"
...
def pb2_generated() -> bool:
    ...
    return (protocol / "voc_pb2.py").exists() and (protocol / "voc_pb2_grpc.py").exists()
...
def run_pipeline_with_params(filters: Optional[List[str]], task: str = "both",
    ...
    return run_async(rt.run_with_params(
        ...
        csv_path=csv_path or str(VOC_DATA_ROOT / "voc.csv"), timeout=timeout,
    ))
