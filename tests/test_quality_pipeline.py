...
엔드투엔드로 정상 동작하는지 확인합니다. 전체 배치 실행은 `python -m quality.quality_pipeline`로 별도 수행하세요."""
import json

from quality.quality_pipeline import TEST_CASE_FILE, load_test_cases, run_pipeline

SAMPLE_SIZE = 2


def test_pipeline_generates_reports(tmp_path, monkeypatch):
    import quality.quality_pipeline as pipeline
    ...
    assert (tmp_path / "docs" / "final_quality_report.md").exists()
