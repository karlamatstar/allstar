...
LOG_DIR = ROOT / "logs" / "services" / "launcher"
LOG_DIR.mkdir(parents=True, exist_ok=True)

AI_TESTS = [
    ("Smoke Test", ["k6", "run", "performance/smoke_test.js"], False),
    ("Load Test", ["k6", "run", "performance/load_test.js"], True),
    ("Random Test", ["k6", "run", "performance/random_test.js"], True),
    ("Stress Test", ["k6", "run", "performance/stress_test.js"], True),
    ("Spike Test", ["k6", "run", "performance/spike_test.js"], True),
    ("검증 테스트", [PY, "-u", "scripts/run_validation_tests.py"], True),
    ("API 종합 성능 테스트", [PY, "-u", "scripts/run_performance_tests.py"], True),
    ("API 끊김 방어 테스트", [PY, "-u", "scripts/run_api_disconnect_test.py"], True),
]
...
class QAControl(tk.Tk):
    def __init__(self):
        ...
        voc_tabs.add(TestTab(voc_tabs, "전체 pytest", [PY, "-u", "-m", "pytest", "voc/quality_diagnosis", "-v"]), text="전체 pytest")
        voc_tabs.add(TestTab(voc_tabs, "단위 테스트", [PY, "-u", "-m", "pytest", "voc/quality_diagnosis/test_agent_unit.py", "voc/quality_diagnosis/test_llm_judge.py", "-v"]), text="단위 테스트")
        for profile_id in "ABCD":
            command = [PY, "-u", "scripts/run_voc_profile.py", "--profile", profile_id]
