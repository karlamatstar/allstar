...
현재 기준일은 2026-07-20이며, 통합 화면의 사용자 실행은 현재 등록된 테스트케이스 전체를 사용한다.
...
오른쪽의 `K6 부하 테스트`, `AI 에이전트 테스트케이스`, `VOC 테스트케이스` 상위 탭은 각각 비밀번호를 확인한 뒤 실제 화면을 표시한다. 기본 비밀번호는 로컬 시연용 `1234`이며 `DASHBOARD_TEST_TABS_PASSWORD` 환경변수로 변경할 수 있다. 한 탭의 잠금을 풀어도 다른 테스트 탭은 별도로 확인하며 브라우저 세션이 끝나면 다시 잠긴다.
