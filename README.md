...
채팅 입력창 아래에는 테스트케이스 한 건을 무작위로 골라 실행하는 장애 시험 버튼 3개가 있다. `503 서비스 이용 불가 시험`, 10초 뒤 응답하는 `504 시간 초과 시험`, `portfolio-api`만 실제 중단하는 `채팅 서버 중단 시험`이다. 버튼 위에는 세 시험이 외부 AI API를 호출하지 않아 API 비용이 발생하지 않는다는 안내를 표시한다. 장애 결과는 실제 대화·채점 로그와 최신 보고서에 남지만 AI 품질 실패가 아니므로 점수와 판정은 `N/A`로 기록한다. 서버 중단 후에는 채팅창의 `채팅 서버 재접속` 버튼으로 복구한다.
