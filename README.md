...
Prometheus가 수집한 요청 수, 오류율, 응답시간, Judge 판정·점수·처리시간과 A~D 테스트케이스 결과를 표시한다. 강제 503·504·서버 다운도 실제 서비스 장애와 같은 retry/unavailable 및 오류율에 포함하되 품질 보고서에서는 `N/A`로 구분한다. 서버 재시작 뒤 마지막 활동 시각은 누적 로그에서 복원하고, 데이터가 없을 때는 수십 년의 잘못된 경과시간 대신 수집 전 상태를 안내한다. Grafana 화면은 새 창으로도 열 수 있다.
...
- 외부 AI 실행 경로를 제외한 최신 자동 회귀: 258개 통과, 환경 조건 1개 건너뜀, 2개 선택 제외
