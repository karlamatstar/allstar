...
32. `TEST_SUITE_CONSOLIDATION.md`
   - 중복 자동검사 통합 기준, 316개에서 288개로 줄인 범위와 전체 비API 회귀 결과
33. `DASHBOARD_TEST_TAB_PASSWORD.md`
   - K6·AI 에이전트·VOC 테스트케이스 상위 탭의 개별 비밀번호 잠금과 환경변수 설정 기준
...
- 전체 자동검사 수집 `293개`, 실제 외부 AI 실행 파일을 제외한 최신 비API 회귀 `286 passed, 3 skipped, 2 deselected` 확인
...
- AI·VOC 날짜별 로그와 최근 활동 5개 원본 보존·과거 GZIP 압축, 테스트케이스 완료 실행 압축과 보고서 제외 구현 완료
- 대시보드 의미 기반 버튼 색상·강조 채팅 입력, 테스트케이스 관리 펼침 자동 이동과 AI·VOC·K6 360px 자동 스크롤 터미널 구현 완료
- 중복 자동검사를 기능별 기존 검사와 목록형 진단으로 통합해 전체 수집 `316개 → 288개`로 정리하고 전체 비API 회귀 완료
- 별도 빠른·기능별·전체 단계 체계는 만들지 않고 작업 마무리 시 전체 비API 회귀를 한 번 실행하는 정책 확정
- K6·AI 에이전트·VOC 테스트 상위 탭은 각각 시연용 비밀번호를 확인한 뒤 실제 기능을 렌더링하도록 잠금 적용
