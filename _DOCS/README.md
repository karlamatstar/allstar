...
- Python 문법 검사와 외부 AI 실행 경로를 제외한 최신 비API 회귀 `250개 통과·1개 환경 제외·2개 선택 제외`
...
현재 K6 대시보드 후속 개선, VOC 보고서 순서, 인위적 챗봇 장애 유발 기능 제거와 이번 문서 정리는 작업 트리에 반영됐으며 아직 커밋·푸시 전이다.

QA 컨트롤러 AI 테스트케이스 탭, 직접 부하 K6 보고서 범위 분리, AI·VOC 테스트케이스 정상 완료 보고서 보호를 구현·비API 검증했다. 현재 미완료 항목은 일부 운영 상세 지표, 장시간·다중 사용자 조건, AWS 배포 보안 검토다. 정확한 항목은 `PROJECT_PROGRESS_CHECKLIST.md`를 따른다.
