...
- 실시간 VOC 챗봇도 테스트케이스와 동일한 9항목·100점 `judge_rubric.json`을 사용한다.
- 실시간 전용 4항목·20점 채점은 승인된 기준이 아니므로 유지하지 않는다.
- 검색 결과 0건은 제한적 재검색 후 `관련 데이터 없음`과 시스템 장애를 구분하고, Judge 점수는 N/A로 기록한다.
- 2026-07-17에 확인한 현재 코드의 채점 불일치와 검색 0건 실패 개선 기준은 `VOC_CHATBOT_QUALITY_AND_FAILURE_IMPROVEMENT_PLAN.md`를 따른다.
...
사용자가 GUI·대시보드에서 A~D 실험군을 실행하면 등록된 전체 테스트케이스를 하나의 실행과 하나의 정식 보고서로 합친다. 사례별 정식 보고서를 별도 폴더로 나누지 않는다. 실행 로그는 `run_id`별로 누적하고 프로필별 Markdown·CSV·JSON·그래프·manifest는 최신 실행으로 덮어쓴다. 명령행 검증은 필요할 때 `--case-id`로 사례 범위를 명시적으로 선택할 수 있으며 2건으로 고정하지 않는다. 세부 구현 기준은 `VOC_TESTCASE_REPORT_AUTOMATION.md`를 따른다.
...
- 상위 `보고서 모음` 탭 아래에 신규 VOC 챗봇 보고서와 VOC 테스트케이스 보고서를 추가해 총 6개 보고서 하위 탭을 사용한다.
...
| `voc_testcase_latest_average_score` | profile | A~D 최신 정상 평가 평균 |
| `voc_testcase_latest_case_results` | profile,result | PASS·REVIEW·FAIL·N/A 분포 |
| `voc_testcase_latest_duration_seconds` | profile | A~D 최신 실행 처리시간 합계 |
| `voc_testcase_latest_case_average_duration_seconds` | profile | A~D 케이스 평균 처리시간 |
| `voc_testcase_latest_case_score` | profile,case_id,result | 테스트케이스별 최신 점수 |
| `voc_testcase_latest_criterion_achievement_percent` | profile,criterion | 항목별 평균 달성률 |
...
AI 에이전트 실시간 운영과 K6 성능 부하 시험 Grafana UID는 유지하되 실제 채팅과 부하 시험 지표는 분리한다. VOC는 운영 흐름과 QA 결과를 한 화면에 섞지 않고 두 개의 대시보드로 나눈다.

#### 10.3.1 VOC 챗봇 실시간 운영 대시보드
...
위 표는 장기 확장 설계다. 2026-07-17 현재 구현은 요청 처리량·p95·최근 및 누적 요청·마지막 활동, Judge 처리 상태·최종 판정·100점 평균·p95 채점시간의 9개 패널이며 단계별 세부 지표는 후속 범위다.
...
QA 실행 프로세스는 종료 후 사라지므로 프로세스 메모리 지표를 직접 사용하지 않는다. 2026-07-17에 계속 실행되는 VOC API가 공유 `_OUTPUT`의 A~D 최신 정식 JSON 보고서를 읽어 `voc_testcase_*` 지표로 노출하도록 구현했다. 별도 Pushgateway는 추가하지 않았으며 Prometheus 시계열은 Docker 영구 볼륨에 저장한다.
...
통합 Streamlit에는 기존 포트폴리오의 Grafana 탭을 확장한 상위 `모니터링` 탭을 하나 두고, 선택하면 AI 에이전트·K6·VOC 챗봇·VOC QA 4개 하위 탭이 바로 표시되게 한다. 중간 탭을 추가하지 않는다. 화면 구조, UID, 빈 상태와 완료 기준은 `INTEGRATED_GRAFANA_DASHBOARD_REQUIREMENTS.md`를 따른다. 2026-07-17에 VOC Grafana JSON 2개와 통합 화면 코드를 구현했고, A~D 테스트케이스 전용 지표·9개 QA 패널·실데이터·재시작 보존까지 검증했다.
...
2026-07-18 현재 결정: 서버 서비스와 Streamlit을 Docker로 실행하고 K6는 전용 Runner 컨테이너로 분리한다. Server Control Center가 두 서비스를 포함한 Compose 전체를 시작·종료하며, Streamlit 행의 접속 버튼으로 `http://localhost:8501`을 연다. 이 구성은 로컬 Docker Desktop에서 사용하며 AWS 배포는 필수가 아니다. 상세 기준은 `DOCKER_STREAMLIT_K6_RUNNER.md`를 따른다.
...
| K6 Runner API | 8200 | 로컬 호스트 전용 신규 |
...
2026-07-18 이후 Streamlit과 K6 Runner도 Docker 서비스로 관리한다. Windows 호스트 `subprocess` 방식은 이전 실행본의 잔류 Streamlit 정리에만 남긴다.
...
`장애·기능 검증 시험`의 기본 비API 회귀는 외부 AI 호출 파일을 제외한다. 실제 AI 품질 시험은 화면 또는 명령에서 대상 범위, 모델과 예상 호출량을 명시해 실행하며 제품 정책상 2건으로 고정하지 않는다. GUI·대시보드의 테스트케이스 실행은 현재 등록된 전체 사례를 사용한다.
...
- 실제 API 속도를 검증할 때는 비교할 사례 범위를 동일하게 맞추고 모델·추론 설정·호출 횟수를 실행 전에 확인한다.
...
과거 20건 구성의 A~D 전체 80건 실행은 약 48분 이상 걸렸다. 현재는 프로필별 10건, A~D 전체 40건이지만 모델 응답시간에 따라 여전히 오래 걸릴 수 있으므로 별도 프로세스와 취소 기능을 사용한다.

GUI 또는 대시보드의 실행 버튼은 실행 시점에 등록된 테스트케이스 전체를 사용한다. 실행 전에는 전체 케이스 수, 실제 AI 평가 대상 수, 프로필과 예상 호출 범위를 확인창에 표시한다. 명령행 검증은 필요에 따라 사례를 명시적으로 선택할 수 있지만 고정된 2건 정책은 사용하지 않는다.
...
- 모델별 실제 속도를 비교할 때는 같은 테스트케이스 범위와 동일한 실행 조건으로 처리시간과 점수 안정성을 비교한다. 측정 조건이 다르면 특정 모델이 더 빠르다고 구현 완료 문서에 단정하지 않는다.
...
- 결정 완료: API, VOC 에이전트, Prometheus, Grafana, Streamlit과 K6 Runner를 Docker로 실행한다.
- Server Control Center가 현재 프로젝트 Compose 서비스를 함께 시작·종료한다.
