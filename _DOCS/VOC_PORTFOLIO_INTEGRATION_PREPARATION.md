...
- 참고 기준선:
  - `VOC`
- 중요: 이 문서는 구현 완료 보고서가 아니다. 요구사항과 구현 순서를 확정하기 위한 준비 문서다.
...
각 실험군은 대표 사례 `TC-01`, `TC-02`를 하나의 실행과 하나의 정식 보고서로 합친다. 사례별 정식 보고서를 별도 폴더로 나누지 않는다. 실행 로그는 `run_id`별로 누적하고 프로필별 Markdown·CSV·JSON·그래프·manifest는 최신 실행으로 덮어쓴다. 세부 구현 기준은 `VOC_TESTCASE_REPORT_AUTOMATION.md`를 따른다.
...
- 화면 너비가 부족한 경우 7개를 무조건 한 줄에 압축하지 않고, PC에서는 `4개 + 3개`, 작은 화면에서는 세로 배치나 가로 스크롤을 사용한다.
...
- 신규 VOC 챗봇 보고서와 VOC A~D 테스트케이스 보고서를 추가해 총 6개 최상위 보고서 탭을 사용한다.
- VOC A~D는 하나의 최상위 탭 안에서 A·B·C·D 개별 결과와 종합 비교를 선택한다.
...
통합 구현 시 `장애·기능 검증 시험`이 실제 AI API를 호출하는 품질 테스트케이스는 상위 지침대로 대표 2개만 사용하도록 고정한다. 현재 `test_quality_pipeline.py`의 `SAMPLE_SIZE=2`는 유지하고, 다른 테스트가 숨은 대량 API 호출을 추가하지 않는지 확인한다.
...
- 실제 API 속도 검증은 대표 케이스 2개만 사용하고, 모델·추론 설정·호출 횟수를 실행 전에 확인한다.
...
개발·통합 검증 중에는 AI API를 실제로 호출하는 대표 테스트케이스를 최대 2개로 제한한다. A~D를 비교해야 할 때도 각 실험군에 동일한 2개 케이스를 재사용한다. 20건 또는 전체 세트는 사용자가 별도로 승인한 최종 검증에서만 실행한다. 외부 AI API를 호출하지 않는 오프라인 단위 테스트는 이 제한에 포함하지 않는다.
...
- 모델별 실제 속도는 아직 같은 조건으로 측정하지 않았으므로 구현 검증에서는 대표 케이스 2개만 사용해
