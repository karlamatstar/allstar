...
통합 Streamlit에는 기존 포트폴리오의 Grafana 탭을 확장한 상위 `모니터링` 탭을 하나 두고, 선택하면 기존 2개와 신규 VOC 2개가 총 4개 하위 탭으로 바로 표시되게 한다. 중간 탭을 추가하지 않는다. 화면 구조, UID, 빈 상태와 완료 기준은 `INTEGRATED_GRAFANA_DASHBOARD_REQUIREMENTS.md`를 따른다. 2026-07-17에 VOC Grafana JSON 2개와 통합 화면 코드를 구현했고, A~D 테스트케이스 전용 지표·8개 QA 패널·실데이터·재시작 보존까지 검증했다.
