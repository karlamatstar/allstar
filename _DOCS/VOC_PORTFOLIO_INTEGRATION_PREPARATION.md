...
- 실시간 대화 리포트 생성 버튼
- A/B/C/D 개별 결과 선택 및 조회
- 존재하는 A~D 결과 표시
- 2개 이상일 때 교차검증 종합 리포트 생성 버튼 활성화
- 최신본과 이력본 분리
...
| 6 | `검증 테스트` | QA 자동화 실행 및 결함 보고서 확인 | 모든 사용자 사용 |
| 7 | `API 종합 성능 테스트` | API 성능 실행 및 성능 보고서 확인 | 모든 사용자 사용 |
| 8 | `API 끊김 방어 테스트` | API 단절 상황의 방어·오류 처리 검증 | 모든 사용자 사용 |

- 각 탭의 설명, VU·실행시간 입력, 시작 버튼, 탭별 콘솔, 보고서 버튼을 기존과 동일하게 유지한다.
- 로컬 통합 프로젝트에서는 원본 코드·pytest를 사용할 수 있으므로 `검증 테스트` 탭을 포함한다.
...
- 원격 QA EXE에서 로컬 소스가 필요한 `검증 테스트`를 누르면 클라이언트가 직접 pytest를 실행하지 않고
...
Center EXE에서 모든 테스트 탭을 볼 수 있고 고부하·장애·검증 테스트를 모두 실행할 수 있다.
...
- `검증 테스트`는 결함 Markdown·Word 보고서 생성까지만 자동 수행한다.
...
- 결정 완료: 현재 로컬·수업 환경에서는 고부하·장애·검증 테스트를 포함한 모든 QA 기능을 모든 사용자에게 제공한다.
