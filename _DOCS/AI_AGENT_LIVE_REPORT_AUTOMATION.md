...
- 대시보드의 명시적 503·504·채팅 서버 중단 시험도 정상 챗봇 대화·채점 로그에 남기되, 총점과 5개 품질 항목 점수를 비워 둔 `N/A`로 기록한다.
- 장애 시험의 `fault_type`, HTTP 상태, 선택된 테스트케이스와 기술 오류를 보고서 확인 목록에 표시한다.
- 장애 시험 N/A 행은 PASS·REVIEW·FAIL 비율과 품질 평균 계산에서 제외한다.
...
## 2026-07-18 명시적 장애 시험 검증

- 503 시험이 즉시 실제 HTTP 503을 반환하고 일반 대화 로그 1건·N/A 채점 로그 2건·최신 보고서를 갱신하는 것을 확인했다.
- 504 시험이 약 10초 후 실제 HTTP 504를 반환하고 같은 저장 흐름을 사용하는 것을 확인했다.
