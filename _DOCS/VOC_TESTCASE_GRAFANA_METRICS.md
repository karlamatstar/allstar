...
상세 질문, 실제 7단계 출력, 채점 근거는 `보고서 모음`과 `VOC 테스트케이스` 상세 화면에서 확인한다.
...
- 기존 A·B·C·D 최신 보고서를 재실행하지 않고 수집했으며 2026-07-17 전체 점검 시 평균 점수는 A 72.125, B 65.5, C 65.0, D 81.0으로 Prometheus에서 확인했다.
- Grafana UID `voc-qa-abcd`에 9개 패널을 자동 배포했다. 브라우저에서 최신 평균 점수와 PASS·REVIEW·FAIL·N/A 분포, 점수·처리시간 시계열 범례를 확인했고, 2026-07-17 최신 저장 케이스 수는 A·B·C·D 모두 10건으로 조회됐다.
