...
- `AI Agent QA AllStar` Streamlit 통합 화면의 왼쪽 4개·오른쪽 3개 상위 탭 구조와 K6 부하 테스트 실행 화면
...
- VOC A~D 화면 실행은 등록된 전체 테스트케이스를 사용하고 명령행 검증은 필요할 때 `--case-id`로 명시적인 사례 범위를 선택할 수 있게 실행 범위 분리
...
- 통합 K6 화면의 4열·2열·1열 반응형 카드, 행별 동일 높이와 하단 실행 버튼 정렬
- K6 입력의 가상 인원 1~999명·시간 10~600초 자동 보정, `[-] [숫자] [+]` 배치와 길게 누르기 자동 증감
- K6 고정 높이 터미널의 최신 출력 자동 스크롤, 사용자 중지와 실패 시 마지막 정상 보고서 보호
- QA 각 시험의 목적·진행 방식·확인 항목·자동 결과 생성을 안내하는 개별 설명과 GUI 보고서 경로 숨김
- QA 전체 14개 탭 공통 실행 잠금, 비동기 중지, 구조화 실행 상태 로그, 실제 AI 테스트 기본 제외
- `.bat` 및 숨김 실행용 `.vbs` 런처
- AI 에이전트 챗봇의 특수어 기반 503·504 강제 장애 규칙, 공개 강제 연결 끊김 필드와 QA 전용 연결 끊김 방어 시험 제거
- AI 에이전트 챗봇 입력창 아래의 명시적 503·504·실제 채팅 서버 중단 버튼과 무작위 테스트케이스 자동 입력
- 장애 시험의 일반 대화·채점 로그·최신 보고서 N/A 기록, `portfolio-api` 단독 중단과 채팅창 재접속 복구
- 서버 중단 버튼의 단일 클릭 실행, 사용자 질문 즉시 표시, 답변 말풍선 대신 중앙 시스템 경고·재접속 버튼 표시
- AI `portfolio-api`와 VOC `voc-api`의 Health를 각 챗봇 화면에서 독립 감지하고, 중단된 화면만 입력·관련 버튼 잠금과 중앙 재접속 UI로 전환
- VOC에는 인위적 중단 시험 버튼을 두지 않고 실제 서버 중단 감지와 `voc-api` 재접속만 제공
...
QA 관리 GUI의 14개 시험은 공통 실행 기록 자동화를 사용한다. 실행별 원문 로그와 k6 원본 요약은 누적한다. 직접 부하 K6 5종은 Prometheus·Grafana 중심으로 운영해 별도 사용자용 Markdown·manifest를 만들지 않고, 나머지 대상 시험은 실행 요약과 최신 manifest를 갱신한다. 장애·기능 검증과 서버 연결 성능 종합 시험의 기존 정식 보고서는 유지한다. 세부 기준은 `QA_REPORT_AUTOMATION.md`에 기록한다.
...
공통 QA Markdown은 정식 품질 보고서가 아니라 실행 상태와 증적을 연결하는 보조 요약이다. VOC A~D 테스트케이스는 사용자가 화면에서 프로필을 실행할 때 등록된 전체 테스트케이스를 단일 Judge 프로세스로 처리하고, 실행 JSON 로그에서 프로필별 Markdown·CSV·JSON·PNG 그래프를 재생성한다. 명령행에서는 `--case-id`로 필요한 사례를 명시적으로 선택할 수 있다. 프로필 결과가 2개 이상이면 A~D 종합 비교 보고서를 최신 결과로 갱신한다. 세부 기준은 `VOC_TESTCASE_REPORT_AUTOMATION.md`를 따른다.
...
- 명령행 검증은 `--case-id`를 반복 지정해 필요한 사례 범위를 명시적으로 선택 가능
...
- K6 상위 탭과 AI 관리·실행 분리 추가 전 당시 브라우저에서 상위 6개, AI 챗봇 하위 5개, 모니터링 4개, 리포트 6개, AI 테스트케이스 4개, VOC 테스트케이스 2개 하위 탭 확인
...
### 2026-07-17 K6 대시보드·인위적 장애 제거 최종 검증

- 상위 탭을 7개로 확장하고 K6·AI 테스트케이스·VOC 테스트케이스 순서를 실제 브라우저에서 확인
- K6 카드의 넓은 화면 4개·3개, 중간 화면 2열, 모바일 1열과 행별 동일 높이·실행 버튼 하단 정렬 확인
- 가상 인원 1~999명·실행 시간 10~600초 보정, `[-] [숫자] [+]` 배치, 길게 누르기 자동 증감과 최신 터미널 출력 자동 스크롤 확인
- 사용자가 999명·600초 설정으로 직접 실행해 초반 연결 거부 1,764건 뒤 HTTP 200 10,392건을 관찰하고 29.8초에 중단함. 정상 완료 성능 기준에는 포함하지 않음
- 특수어 기반 503·504 강제 장애 규칙, 공개 강제 연결 끊김 요청 필드와 QA 전용 연결 끊김 방어 시험 제거
- 관련 집중검사 `39개 통과`, 외부 AI 실행 경로를 제외한 최신 비API 회귀 `250개 통과·1개 건너뜀·2개 선택 제외`
- AI 서버 컨테이너를 다시 빌드해 `/health` 200과 공개 `ChatRequest` 스키마를 확인했으며 외부 AI API는 호출하지 않음

### 2026-07-18 명시적 챗봇 장애 시험 구현·검증

- 일반 질문의 특수어 장애 규칙은 제거된 상태를 유지하고, 사용자가 직접 누르는 대시보드 버튼으로만 장애를 시작하도록 분리
- 현재 AI 테스트케이스 중 한 건을 무작위 선택해 질문을 자동 입력하며 실제 AI 생성·Judge API는 호출하지 않음
- 503 즉시 응답, 504 약 10초 지연 응답을 실제 HTTP 상태로 확인하고 일반 대화 로그·모델별 N/A 채점 로그·최신 보고서 자동 갱신 확인
- 실제 `portfolio-api` 단독 중단 후 AI `/health` 연결 실패, VOC·Prometheus·Grafana 유지, 재접속 후 `/health` 200 복구 확인
- 실제 `voc-api` 단독 중단 시 AI Health 200 유지와 VOC 전용 잠금·재접속을 확인하고, 반대 방향에서도 VOC Health 200 유지와 AI 전용 잠금·재접속을 확인
- 서버 중단·재접속 수명주기는 별도 장애 이벤트 JSONL에 누적하고 품질 집계에서는 N/A를 FAIL·0점으로 변환하지 않음
- 집중 테스트 `23개 통과`, 외부 AI 실행 경로 제외 전체 비API 회귀 `258개 통과·1개 건너뜀·2개 선택 제외`
