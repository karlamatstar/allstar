...
- 왼쪽 4개·오른쪽 2개 상위 탭, 모니터링 4개·리포트 6개 하위 탭, AI·VOC 테스트케이스와 VOC 7단계 상세 화면은 `INTEGRATED_DASHBOARD_STRUCTURE_REQUIREMENTS.md`에 요구사항만 확정했으며 아직 실제 코드에는 반영하지 않았다.
...
- 현재 데이터 기준 전체 20건과 실제 LLM 평가 대상 18건을 확인창과 실행 설정에 표시
...
3. 통합 대시보드의 왼쪽 4개·오른쪽 2개 상위 구조 구현
4. AI·VOC 챗봇과 테스트케이스 화면 이식·완성
5. 모니터링 Grafana 4개와 리포트 6개 하위 탭 구현
6. 반응형·빈 상태 검증
7. A 프로필 대표 2건 독립 평가 재검증
