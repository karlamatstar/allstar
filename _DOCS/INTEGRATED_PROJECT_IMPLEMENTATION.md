...
> 상태: **통합 기능 구현·최신 비API 회귀·주요 실제 화면 검증 완료, 배치 A~D 대표 재검증 등은 남은 작업으로 분리**
...
- 과거 A 프로필 배치 TC-01·TC-02는 생성 파이프라인만 완료되고 당시 Anthropic 401로 Judge가 N/A였다. 현재 인증 상태로 같은 2건을 재검증하는 작업은 남아 있다.
- GUI·대시보드의 A~D 전체 10개 테스트케이스 실제 실행은 비용이 크므로 수행하지 않았다. Codex 검증은 사용자 승인 후 동일 대표 2건만 사용한다.
