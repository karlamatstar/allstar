...
- 모니터링의 Grafana 하위 탭 4개와 리포트 모음의 보고서 하위 탭 6개
- AI 에이전트 테스트케이스의 관리·전체 실행·배치 품질·유형 비교·상세 화면
- VOC 테스트케이스의 관리·실 테스트, A~D 전체 실행과 사례별 7단계 결과 화면
...
- 모바일 VOC 챗봇·실 테스트 A~D 카드의 고정 높이 제거와 내용 기준 높이
- 리포트 모음의 VOC 프로필 하위 탭을 `교차 테스트 (A)`부터 `교차 테스트 (D)`까지로 명확화
...
- 신규 VOC Grafana JSON의 Docker 자동 프로비저닝과 실제 Prometheus 데이터 패널 값은 서버 전체 실행 후 확인한다.
- GUI의 A~D 전체 테스트케이스 실제 실행은 비용이 발생하므로 수행하지 않았다.
...
- VOC 실 테스트 상태 배지는 카드 밖의 공통 높이 영역에 표시하고 카드는 데스크톱 17rem·1200px 이하 19rem 높이를 사용한다. 모바일에서는 비활성 빈 상태 영역을 제거한다.
...
- VOC 실 테스트의 현재 전체 10건·실제 AI 대상 9건·장애 전용 1건 안내와 A~D 카드 확인
...
5. 실제 AI Agent·VOC 채팅으로 자동 보고서 갱신 확인
6. A 프로필 대표 2건 독립 평가 재검증
