...
> 후속 구조 결정: 이 문서의 폴더명과 기능 상태는 원본 세 프로젝트를 분석한 2026-07-16 시점의 기록이다. `_Total`의 현재 구조와 구현 상태는 `PROJECT_DIRECTORY_STRUCTURE.md`, `INTEGRATED_PROJECT_IMPLEMENTATION.md`, `PROJECT_PROGRESS_CHECKLIST.md`를 기준으로 하며, 이 문서의 `미구현` 표현을 현재 통합 프로젝트 상태로 해석하지 않는다.
...
- 통합 전 원본에는 질문의 `퇴근` 또는 `강사` 단어로 장애를 발생시키는 테스트 규칙이 실제 `/chat` 경로에 포함되어 있었다. `_Total`에서는 2026-07-17에 해당 규칙과 인위적 연결 끊김 시험을 제거했다.
...
> 위 문장은 원본 `VOC`·`voc_upgrade` 분석 당시 상태다. 후속 `_Total`에는 A~D 선택, 7단계 결과, 대화 로그, 9항목·100점 품질 화면과 자동 보고서를 갖춘 VOC Streamlit 챗봇이 구현되어 있다.
