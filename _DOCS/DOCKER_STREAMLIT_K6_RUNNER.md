...
Docker 내부 Streamlit에는 Docker 소켓을 연결하지 않았다. 따라서 AI 챗봇의 명시적 `채팅 서버 중단`·`재접속` 버튼이 호스트의 Compose 서비스를 직접 제어하는 기능은 Docker 화면에서 안전상 제한된다. 일반 채팅, 503·504 장애 기록, K6 시험과 서버 관리 GUI의 전체·개별 서비스 제어는 유지된다. 이 기능을 Docker 화면에서도 제공하려면 Docker 소켓을 직접 노출하지 말고, 허용 서비스만 제어하는 별도 인증 운영 API를 설계해야 한다.
