...
from app.jira_client import JIRA_URL, JIRA_USER, JIRA_API_TOKEN, JIRA_PROJECT_KEY

def run_k6():
    print("▶ Running Chaos Test (K6)...", flush=True)
    script = PROJECT_ROOT / "performance" / "chaos_test.js"
    result_file = PROJECT_ROOT / "performance" / "results" / "chaos_result.json"
...
def create_chaos_defect_report(k6_out, pytest_out):
    ...
    report_dir = PROJECT_ROOT / "quality" / "reports" / "defects" / "chaos"
...
현상 파악 후 결함 보고서(quality/reports/defects/chaos/defect_report.md)를 작성하고,
